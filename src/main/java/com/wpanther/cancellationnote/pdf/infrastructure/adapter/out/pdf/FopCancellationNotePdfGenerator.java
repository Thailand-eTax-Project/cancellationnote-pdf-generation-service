 ...
 *
 * This class transforms XML cancellation note data using an XSL-FO stylesheet
...
@Component
@Slf4j
public class FopCancellationNotePdfGenerator {

    private static final String FOP_CONFIG_PATH = "fop/fop.xconf";
    private static final String CANCELLATION_NOTE_XSL_PATH = "xsl/cancellationnote-direct.xsl";
    ...
    public FopCancellationNotePdfGenerator(
        ...
        try {
            ...
            this.cachedTemplates = compileTemplates(tf, CANCELLATION_NOTE_XSL_PATH);
            ...
            this.pdfSizeSummary = DistributionSummary.builder("pdf.fop.size.bytes")
                    .description("Size of generated PDFs in bytes")
            ...
            log.info("FopCancellationNotePdfGenerator initialized: maxConcurrentRenders={} maxPdfSizeBytes={} (each FOP render ~50-200 MB heap)",
    ...
    /**
     * Generate PDF from signed XML data using the cancellation note XSL-FO template.
     ...
     *
     * @param xmlData The signed XML (rsm:CancellationNote_CrossIndustryInvoice)
     * @param params  XSLT parameters to set on the transformer (may be null)
     * @return PDF bytes
     * @throws CancellationNotePdfGenerationException if generation fails
     */
    @NewSpan("pdf.fop.render")
    public byte[] generatePdf(String xmlData, Map<String, Object> params) throws CancellationNotePdfGenerationException {
        log.debug("Awaiting render permit (available={})", renderSemaphore.availablePermits());
        try {
            renderSemaphore.acquire();
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new CancellationNotePdfGenerationException("PDF generation interrupted while waiting for render slot", e);
        }
        long t0 = System.nanoTime();
        try {
            log.debug("Generating PDF with cached template: {}", CANCELLATION_NOTE_XSL_PATH);
            ...
            throw new CancellationNotePdfGenerationException("Failed to create transformer from cached templates: " + e.getMessage(), e);
        } finally {
            try {
     ...
     */
    public byte[] generatePdf(String xmlData) throws CancellationNotePdfGenerationException {
        return generatePdf(xmlData, null);
    }

    private byte[] renderPdf(String xmlData, Transformer transformer) throws CancellationNotePdfGenerationException {
