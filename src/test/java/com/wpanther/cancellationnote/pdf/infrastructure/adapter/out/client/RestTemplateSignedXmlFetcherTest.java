package com.wpanther.cancellationnote.pdf.infrastructure.adapter.out.client;
...
class RestTemplateSignedXmlFetcherTest {
    ...
    void fetch_success_returnsXml() {
        mockServer.expect(requestTo("http://minio/xml"))
                ...
                .andRespond(withSuccess("<CancellationNote/>", MediaType.APPLICATION_XML));

        String result = fetcher.fetch("http://minio/xml");

        assertThat(result).isEqualTo("<CancellationNote/>");
        mockServer.verify();
    }
