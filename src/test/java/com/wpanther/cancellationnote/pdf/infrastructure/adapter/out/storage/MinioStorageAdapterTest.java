...
class MinioStorageAdapterTest {
    ...
    @BeforeEach
    void setUp() {
        adapter = new MinioStorageAdapter(s3Client, "cancellationnotes", "http://localhost:9000/cancellationnotes", meterRegistry);
    }
    ...
    void store_uploadsAndReturnsS3Key() {
        ...
        String key = adapter.store("CN-001", new byte[]{1, 2, 3});

        assertThat(key).matches("\\d{4}/\\d{2}/\\d{2}/cancellationnote-CN-001-.+\\.pdf");
    ...
    void resolveUrl_prependsBaseUrl() {
        ...
        assertThat(url).isEqualTo("http://localhost:9000/cancellationnotes/2024/01/15/file.pdf");
    }
    ...
    void store_preservesThaiCharacters() {
        ...
        assertThat(key).contains("cancellationnote-INV-ไทย-001-");
        verify(s3Client).putObject(any(PutObjectRequest.class), any(RequestBody.class));
    }
    ...
    void store_sanitizesProblematicCharacters() {
        ...
        assertThat(key).contains("cancellationnote-INV_test___001-");
        verify(s3Client).putObject(any(PutObjectRequest.class), any(RequestBody.class));
    }
}
