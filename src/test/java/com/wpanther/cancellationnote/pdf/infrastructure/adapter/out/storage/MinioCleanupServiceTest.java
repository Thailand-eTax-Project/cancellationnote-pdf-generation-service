...
import com.wpanther.cancellationnote.pdf.infrastructure.adapter.out.persistence.JpaCancellationNotePdfDocumentRepository;
...
class MinioCleanupServiceTest {
    ...
    @Mock
    private JpaCancellationNotePdfDocumentRepository repository;
    ...
    private MinioCleanupService getCleanupService() {
        try {
            return MinioCleanupService.class
                    .getDeclaredConstructor(MinioStorageAdapter.class, JpaCancellationNotePdfDocumentRepository.class)
    ...
    void testCleanupOrphanedPdfs() {
        ...
        List<String> minioKeys = List.of(
                "2024/01/15/cancellationnote-CN-001-abc123.pdf",
                "2024/01/15/cancellationnote-CN-002-def456.pdf",
                "2024/01/15/cancellationnote-CN-003-orphan.pdf"
        );
        ...
        verify(minioStorage).deleteWithoutCircuitBreaker("2024/01/15/cancellationnote-CN-003-orphan.pdf");
        verify(minioStorage, never()).deleteWithoutCircuitBreaker("2024/01/15/cancellationnote-CN-001-abc123.pdf");
        verify(minioStorage, never()).deleteWithoutCircuitBreaker("2024/01/15/cancellationnote-CN-002-def456.pdf");
    }
    ...
    void testCleanupOrphanedPdfs_AllOrphans() {
        ...
        verify(minioStorage).listAllPdfs();
        verify(repository).findAllDocumentPaths();
        verify(minioStorage).deleteWithoutCircuitBreaker("2024/01/15/cancellationnote-CN-001-abc123.pdf");
        verify(minioStorage).deleteWithoutCircuitBreaker("2024/01/15/cancellationnote-CN-002-def456.pdf");
    }
    ...
    void testCleanupOrphanedPdfs_DatabaseError() {
        // Given
        List<String> minioKeys = List.of("2024/01/15/cancellationnote-CN-001-abc123.pdf");
        when(minioStorage.listAllPdfs()).thenReturn(minioKeys);
