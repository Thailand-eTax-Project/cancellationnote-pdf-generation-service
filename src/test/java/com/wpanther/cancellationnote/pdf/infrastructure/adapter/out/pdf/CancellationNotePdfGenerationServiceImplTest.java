...
@DisplayName("CancellationNotePdfGenerationServiceImpl Unit Tests")
class CancellationNotePdfGenerationServiceImplTest {

    @Mock private FopCancellationNotePdfGenerator fopPdfGenerator;
    @Mock private PdfA3Converter pdfA3Converter;

    private CancellationNotePdfGenerationServiceImpl service;

    private static final String DOC_NUMBER = "CN-2024-001";

    // Minimal signed XML with a GrandTotalAmount of 1070 (CancellationNote namespaces)
    private static final String SIGNED_XML =
        ...
        "  <rsm:ExchangedDocument><ram:ID>CN-2024-001</ram:ID><ram:IssueDateTime>2024-01-15T00:00:00.0</ram:IssueDateTime></rsm:ExchangedDocument>" +
    ...
    @BeforeEach
    void setUp() {
        service = new CancellationNotePdfGenerationServiceImpl(fopPdfGenerator, pdfA3Converter);
    }
    ...
    void generatePdf_success_embedsSignedXmlUnmodified() throws Exception {
        ...
        verify(pdfA3Converter).convertToPdfA3(any(), eq(SIGNED_XML),
                eq("cancellationnote-" + DOC_NUMBER + ".xml"), eq(DOC_NUMBER));
    }

    @Test
    @DisplayName("generatePdf() wraps CancellationNotePdfGenerationException from FOP")
    void generatePdf_fopFails_wrapsException() throws Exception {
        when(fopPdfGenerator.generatePdf(any(), any()))
                .thenThrow(new CancellationNotePdfGenerationException("XSL failed"));
    ...
    @Test
    @DisplayName("generatePdf() wraps CancellationNotePdfGenerationException from PdfA3Converter")
    void generatePdf_pdfA3Fails_wrapsException() throws Exception {
        when(fopPdfGenerator.generatePdf(any(), any())).thenReturn(new byte[100]);
        when(pdfA3Converter.convertToPdfA3(any(), any(), any(), any()))
                .thenThrow(new CancellationNotePdfGenerationException("ICC missing"));
